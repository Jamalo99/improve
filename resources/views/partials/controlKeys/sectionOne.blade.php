<div class="card">
    <div class="card-header cursor-pointer" data-toggle="collapse" data-target="#collapseSectionOne" aria-expanded="false"
        style="cursor: pointer;" aria-controls="collapseSectionOne">
        {{ trans('formulary.controlKey.section.risk-description.title') }}
    </div>
    <div class="collapse" id="collapseSectionOne">
        <div class="card-body">
            <form id="updateControlKeySectionOne" method="POST"
                ...
                <div class="container-fluid">
                    ...
                    <div class="form-group d-flex justify-content-end align-items-center">
                        <svg id="checkSectionOne" xmlns="http://www.w3.org/2000/svg" width="26" height="26"
                    ...
                    <div id="updateDateSectionOne" class="d-flex justify-content-end" style="display: none;"><span
                            id="savedTimeSectionOne"></span>
                    </div>
                </div>
            </form>
        </div>
...
<script>
    $(document).ready(function() {
        //Save header
        $('#updateControlKeySectionOne').on('submit', function(event) {
            event.preventDefault();


            console.log('section one')

            $('#checkSectionOne').hide();
            $('#updateDateSectionOne').hide();

            jQuery.ajax({
                url: "{{ route('frontend.question-answers.update')}}",
                data: jQuery('#updateControlKeySectionOne').serialize() + "&_token={{ csrf_token() }}",
                type: 'PUT',

                success: function() {
                    // Show the SVG on success
                    $('#checkSectionOne').show();

                    // Get the current timestamp
                    var saveTime = Date.now();

                    // Set the value of hidden input and update displayed time
                    $('#savedTimeSectionOne').val(saveTime);
                    $('#savedTimeSectionOne').text(new Date(saveTime).toLocaleString());

                    $('#updateDateSectionOne').show();
