...
@can('protocol_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.protocols.create') }}">
                ...
                <tbody>
                    @foreach($protocols as $key => $protocol)
                        <tr data-entry-id="{{ $protocol->id }}">
                            ...
                            <td>
                                @can('protocol_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.protocols.show', $protocol->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('protocol_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.protocols.edit', $protocol->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('protocol_delete')
                                    <form action="{{ route('admin.protocols.destroy', $protocol->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('protocol_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.protocols.massDestroy') }}",
