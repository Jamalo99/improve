    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.sections.update", [$section->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('control_key_reference') ? 'is-invalid' : '' }}" name="control_key_reference_id" id="control_key_reference_id" required>
                    @foreach($control_key_references as $id => $entry)
                        <option value="{{ $id }}" {{ (old('control_key_reference_id') ? old('control_key_reference_id') : $section->control_key_reference->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('title_it') ? 'is-invalid' : '' }}" type="text" name="title_it" id="title_it" value="{{ old('title_it', $section->title_it) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('title_en') ? 'is-invalid' : '' }}" type="text" name="title_en" id="title_en" value="{{ old('title_en', $section->title_en) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('display_order') ? 'is-invalid' : '' }}" type="number" name="display_order" id="display_order" value="{{ old('display_order', $section->display_order) }}" step="1" required>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('original') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="original" id="original" value="1" {{ $section->original || old('original', 0) === 1 ? 'checked' : '' }}>
