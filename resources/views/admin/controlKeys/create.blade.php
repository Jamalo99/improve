...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.control-keys.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('indice') ? 'is-invalid' : '' }}" type="number" name="indice" id="indice" value="{{ old('indice', '') }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('cadence') ? 'is-invalid' : '' }}" name="cadence_id" id="cadence_id">
                    @foreach($cadences as $id => $entry)
                        <option value="{{ $id }}" {{ old('cadence_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
