        ...
        <div class="col-md-12">
            @can('fraud_question_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.fraud-questions.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $fraudQuestion->id }}">
                                        ...
                                        <td>
                                            @can('fraud_question_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.fraud-questions.show', $fraudQuestion->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('fraud_question_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.fraud-questions.edit', $fraudQuestion->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('fraud_question_delete')
                                                <form action="{{ route('frontend.fraud-questions.destroy', $fraudQuestion->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('fraud_question_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.fraud-questions.massDestroy') }}",
