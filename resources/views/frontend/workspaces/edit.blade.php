    ...
    <div class="row justify-content-center">
        <div class="col-md-12">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <form method="POST" action="{{ route("frontend.workspaces.update", [$workspace->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="user_reference_id" id="user_reference_id" required>
                                @foreach($user_references as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('user_reference_id') ? old('user_reference_id') : $workspace->user_reference->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control" type="text" name="owner" id="owner" value="{{ old('owner', $workspace->owner) }}" required>
                            @if($errors->has('owner'))
                                <div class="invalid-feedback">
                                    {{ $errors->first('owner') }}
                                </div>
