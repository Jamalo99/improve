        ...
        <div class="col-md-12">
            @can('risk_map_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.risk-maps.create') }}">
                            ...
                            <tbody>
                                @foreach($riskMaps as $key => $riskMap)
                                    <tr data-entry-id="{{ $riskMap->id }}">
                                        ...
                                        <td>
                                            @can('risk_map_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.risk-maps.show', $riskMap->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('risk_map_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.risk-maps.edit', $riskMap->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('risk_map_delete')
                                                <form action="{{ route('frontend.risk-maps.destroy', $riskMap->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('risk_map_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.risk-maps.massDestroy') }}",
