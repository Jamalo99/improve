                    ...
                    <form method="POST" action="{{ route("frontend.users.update", [$user->id]) }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            <div>
                                ...
                                <input type="checkbox" name="verified" id="verified" value="1" {{ $user->verified || old('verified', 0) === 1 ? 'checked' : '' }}>
                        ...
                        <div class="form-group">
                            <div>
                                <input type="hidden" name="is_sub_user" value="0">
                                <input type="checkbox" name="is_sub_user" id="is_sub_user" value="1" {{ $user->is_sub_user || old('is_sub_user', 0) === 1 ? 'checked' : '' }}>
                                <label for="is_sub_user">{{ trans('cruds.user.fields.is_sub_user') }}</label>
                            </div>
                            @if($errors->has('is_sub_user'))
                                <div class="invalid-feedback">
                                    {{ $errors->first('is_sub_user') }}
                                </div>
                            @endif
                            <span class="help-block">{{ trans('cruds.user.fields.is_sub_user_helper') }}</span>
                        </div>
                        <div class="form-group">
                            <div>
                                <input type="hidden" name="active" value="0">
                                <input type="checkbox" name="active" id="active" value="1" {{ $user->active || old('active', 0) === 1 ? 'checked' : '' }}>
                                <label for="active">{{ trans('cruds.user.fields.active') }}</label>
                            </div>
